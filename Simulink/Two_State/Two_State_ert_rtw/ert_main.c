...
void rt_OneStep(void)
{
   ...
   */
  Two_State_SetEventsForThisBaseStep(eventFlags);
  ...
  currentTime = (extmodeSimulationTime_T) ((Two_State_M->Timing.clockTick0 * 1)
    + 0)
    ;
  Two_State_step0();
  ...
  if (need2runFlags[1]) {
    ...
    {
     case 1 :
      currentTime = (extmodeSimulationTime_T) ((Two_State_M->Timing.clockTick1 *
        10) + 0)
        ;
      Two_State_step1();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(Two_State_M, 0);
  ...
  Two_State_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(Two_State_M->extModeInfo, &rtmGetTFinal(Two_State_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Two_State_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(Two_State_M);
