...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T)
    Draw_Graph_Flex_Vol_SIMULINK_M->Timing.clockTick0;
  Draw_Graph_Flex_Vol_SIMULINK_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(Draw_Graph_Flex_Vol_SIMULINK_M, 0);
  ...
  Draw_Graph_Flex_Vol_SIMULINK_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(Draw_Graph_Flex_Vol_SIMULINK_M->extModeInfo,
    &rtmGetTFinal(Draw_Graph_Flex_Vol_SIMULINK_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Draw_Graph_Flex_Vol_SIMULINK_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(Draw_Graph_Flex_Vol_SIMULINK_M);
  ...
  /* Terminate model */
  Draw_Graph_Flex_Vol_SIMULINK_terminate();
